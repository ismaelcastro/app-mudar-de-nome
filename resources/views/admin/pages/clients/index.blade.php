    ...
    <div id="kt_app_content_container" class="app-container container-xxl">
        ...
        <div class="list-info-page">
            @if($clients instanceof \Illuminate\Pagination\LengthAwarePaginator )
                <p>Mostrando de {{ $clients->appends($dataForm)->firstItem() }}
                    até {{ $clients->appends($dataForm)->lastItem() }} registros de um total
                    de {{ $clients->appends($dataForm)->total() }}</p>
            @endif
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered verticle-middle">
                ...
                <tr>
                    ...
                    @endif
                    <th scope="col" class="" data-original-title="Ordenar pelo nome" data-toggle="tooltip"
                        data-placement="bottom">
                        @if( isset($dataForm['ord']) && $dataForm['ord']=='name;desc')
                            <a href="javascript:void(0);"
                               onclick="document.getElementById('ord').value='name;asc';document.getElementById('form-searh-interno').submit();"
                               class="text-primary ordenar-desc">Nome</a>
                        @else
                            <a href="javascript:void(0);"
                               onclick="document.getElementById('ord').value='name;desc';document.getElementById('form-searh-interno').submit();"
                               class="text-primary ordenar">Nome</a>
                        @endif
                    </th>
                ...
                @forelse( $clients as $client )
                    <tr>
                        ...
                        <td>
                            <i class="fa fa-user fa-lg color-lead white-tooltip" title=""
                               data-original-title="{{ $client->profile}}" data-toggle="tooltip" data-placement="right"
                               style="color:{{ $profile_color[$client->profile] }}"></i>
                        </td>
                        @if(auth()->user()->id == 1 || auth()->user()->id == 4)
                            <td>
                                {{$client->mautic_id}}
                        ...
                        <td>
                            <a href="{{ route('admin.clients.show', ['client' => $client->id]) }}"
                               title="{{$client->name}}" class="text-primary-inverse">
                                {{$client->name}}
                            </a>
                            {{$client->job}}
                        </td>
                        ...
                        <td>{{$client->ddd}}</td>
                        <td>
                            <a href="https://web.whatsapp.com/send?phone=+55{{$client->ddd}}{{$client->phone_clean2}}&text=Olá, gostaria de mais informações"
                               class="text-dark">
                                {{$client->phone_clean}}
                            </a>
                    ...
                    <span class="action-table">
                        <a href="{{ route('admin.clients.edit', ['client' => $client->id]) }}" title=""
                           data-toggle="tooltip" data-placement="top" data-original-title="editar" class="mr-2">
                            <i class="fa fa-pencil m-r-5"></i>
                        </a>

                        <a href="{{ route('admin.clients.destroy', ['client' => $client->id]) }}" data-toggle="tooltip"
